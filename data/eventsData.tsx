const eventsData = [
  {
    id: 1,
    eventTitle: "Pitch Perfect",
    eventDetails:
      "Pitch Perfect is a singing competition under Abhivyakti 2022 where participants will be judged on their vocals only.",
    participationFee: ["80/-"],
    registerLink: "https://forms.gle/ntYLQsRh2gG9oJGo7",
    rulebookLink: "",
    dataAos: "fade-up"
  },
  {
    id: 2,
    eventTitle: "Abhivyakti Got talent",
    eventDetails:
      "In 5 minutes, participants have to show his/her talent like a magic trick, acting,drama or anything else in a video clip.",
    participationFee: ["80/-"],
    registerLink: "https://forms.gle/bAqNyYotJtdooptC7",
    rulebookLink: "",
    dataAos: "fade-up"
  },
  {
    id: 3,
    eventTitle: "Personality contest",
    eventDetails:
      "Participants will have to choose a theme of their own and dress up according to it.",
    participationFee: ["80/-"],
    registerLink: "https://forms.gle/hVMF9GmwBLLzVkZB9",
    rulebookLink: "",
    dataAos: "zoom-in"
  },
  {
    id: 4,
    eventTitle: "Work it up",
    eventDetails:
      "Students will submit their dance videos which will be shown during the competition on Youtube live.",
    participationFee: ["80/-"],
    registerLink:
      "https://docs.google.com/forms/d/e/1FAIpQLSfXTVogU0Q6Yypk1SfjGfL9H08n6j0ydFGFBV6RBQO_HTjjcw/viewform?usp=sf_link",
    rulebookLink: "",
    dataAos: "zoom-in"
  },
  {
    id: 5,
    eventTitle: "Elocution Battle",
    eventDetails:
      "It’s a platform where candidates get  to explain their policies and ideas.Each speaker gets a set amount of time.",
    participationFee: ["80/-"],
    registerLink: "",
    rulebookLink: "",
    dataAos: "zoom-in"
  },
  {
    id: 6,
    eventTitle: "Meme Making Competition",
    eventDetails:
      "A competition in which participants will be asked to submit the Sarcastic joke made by them .",
    participationFee: ["30/-"],
    registerLink: "https://forms.gle/h6TpV7SFrQqPzbcP8",
    rulebookLink:
      "https://drive.google.com/file/d/1qxH-etHPf4jRyPnbbi8RGpcvaQu_bdzy/view?usp=sharing",
    dataAos: "zoom-in"
  },
  {
    id: 7,
    eventTitle: "QUIZOMANIA",
    eventDetails:
      "An intriguing and fun quiz competition that will test your knowledge.There will be 3 quizzes with specific topics.",
    participationFee: [
      "One Quiz - 30/-",
      "Two Quizzes - 45/-",
      "Three Quizzes - 60/-"
    ],
    registerLink: "https://forms.gle/P7mfatwkMJNhXRCw6",
    rulebookLink: "",
    dataAos: "zoom-in"
  },
  {
    id: 8,
    eventTitle: "Picart",
    eventDetails:
      "A photography enthusiast centric event hosted by the Probe student media body.",
    participationFee: ["50/-"],
    registerLink: "https://forms.gle/FpXaw1dMx9nL4oD27",
    rulebookLink:
      "https://docs.google.com/document/d/1OvgZv4YBeyYrF7YfTtkGHWl67gJZyn8tL7QXGPh_tWg/edit?usp=sharing",
    dataAos: "zoom-in"
  }
];

export default eventsData;
